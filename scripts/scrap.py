...
async def start_scraping(channel):
    """Start scraping and handle authentication errors"""
    logger.info("Scraping data...")

    # Authenticate if not authenticated yet
    try:
        await client.start()  # This will automatically handle the authentication if needed.
    except SessionPasswordNeededError:
        # This happens when a password is required (2FA enabled)
        await client.start(password="Mohammed1?")

    await scrape_telegram_channels(channel)
